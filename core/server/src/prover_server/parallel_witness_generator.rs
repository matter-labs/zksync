...
impl WitnessGenerator {
    ...
    fn should_work_on_block(
        &self,
        block_number: BlockNumber,
    ) -> Result<Option<Block>, failure::Error> {
    ...
    fn maintain(self) {
        ...
        loop {
            ...
            let should_work = match self.should_work_on_block(current_block) {
                ...
                Err(err) => {
                    log::warn!("witness generator block check failed: {}", err);
                    continue;
                }
            };
            if let Some(block) = should_work {
    ...
    for op in ops {
        match op {
            ...
            FranklinOp::TransferFrom(transfer_from) => {
                let transfer_from_witness =
                    TransferFromWitness::apply_tx(&mut witness_accum.account_tree, &transfer_from);
