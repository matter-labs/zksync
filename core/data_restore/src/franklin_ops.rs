...
fn get_franklin_ops_from_block(ops_block: FranklinOpsBlock) -> Result<Vec<FranklinOp>, DataRestoreError> {
    ...
    let mut ops = vec![];
    while (current_pointer < ops_block.commitment_data.len()) {
...
fn get_franklin_ops_block(event_data: &EventData) -> Result<FranklinOpsBlock, DataRestoreError> {
    let transaction =
        get_ethereum_transaction(&event_data.transaction_hash)?;
    let commitment_data = get_commitment_data_from_ethereum_transaction(&transaction)?;
