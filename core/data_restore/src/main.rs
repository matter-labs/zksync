...
fn main() {
    ...
    let infuraEndpointId = u8::from_str(&args[1]).expect("Network endpoint should be convertible to u8");
    info!("Network number is {}", &infuraEndpointId);
    let config = match infuraEndpointId {
        ...
        _ => None,
    }.expect("Its acceptable only 1 for Mainnet and 4 for Rinkeby netwroks");
    let from = U256::from_str(&args[2]).expect("From block should be convertible to u256");
    info!("From block number is {}", &from);
    let delta = U256::from_str(&args[3]).expect("Blocks delta should be convertible to u256");
