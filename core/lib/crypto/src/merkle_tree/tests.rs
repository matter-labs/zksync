...
fn test_verify_proof() {
    ...
    for (idx, item) in elements.iter().enumerate() {
        ...
        assert!(par_tree.verify_proof(idx, *item, par_merkle_path));
        assert!(seq_tree.verify_proof(idx, *item, seq_merkle_path));
    }
    ...
    assert!(!seq_tree.verify_proof(1, elements[0], seq_merkle_path));
}
...
fn simulate_transfer_to_new_par_tree_seq_tree() {
    ...
    assert!(seq_tree.verify_proof(element_idx, element, seq_audit_to_before));
    assert!(par_tree.verify_proof(element_idx, element, par_audit_to_before));
}
