...
async fn initial_price() {
    ...
    for eth_client_price in test_vector {
        ethereum.get_mut_mock().unwrap().set_gas_price(eth_client_price.into()).await.unwrap();
...
async fn initial_upper_gas_limit() {
    ...
    ethereum.get_mut_mock().unwrap().set_gas_price(U256::from(PRICE_LIMIT) + 1).await.unwrap();
    ...
    ethereum.get_mut_mock().unwrap().set_gas_price(U256::from(PRICE_LIMIT) * 2).await.unwrap();
...
async fn gas_price_limit_scaling() {
    ...
    expected_price = ethereum.get_mock().unwrap().get_gas_price().await.unwrap().as_u64();
    for _ in 0..PRICE_UPDATES {
        ...
        ethereum.get_mut_mock().unwrap().set_gas_price(U256::from(expected_price)).await.unwrap();
    }
}
...
async fn gas_price_limit_average_basis() {
    ...
    // Set the client price way beyond the limit.
    ethereum.get_mut_mock().unwrap().set_gas_price(SUGGESTED_PRICE.into()).await.unwrap();
