...
async fn transaction_state() {
    ...
    let current_block = eth_sender.ethereum.get_mock().unwrap().block_number().await.unwrap().as_u64();
...
async fn operation_commitment_workflow() {
    ...
    for (eth_op_id, aggregated_operation) in aggregated_operations.iter().enumerate() {
        ...
        let deadline_block =
            eth_sender.get_deadline_block(eth_sender.ethereum.get_mock().unwrap().block_number().await.unwrap().as_u64());
...
async fn stuck_transaction() {
    ...
    let block_number = U64::from(eth_sender.ethereum.get_mock().unwrap().block_number().await.unwrap().as_u64() + EXPECTED_WAIT_TIME_BLOCKS);
    // Skip some blocks and expect sender to send a new tx.
    eth_sender.ethereum.get_mut_mock().unwrap().set_block_number(block_number).await.unwrap();
    eth_sender.proceed_next_operations().await;
