...
pub(crate) async fn get_unconfirmed_op_by_hash(
    ...
    client.get_unconfirmed_op(eth_tx_hash).await
}
...
async fn handle_get_account_transactions_history(
    ...
    let mut ongoing_ops = get_ongoing_priority_ops(&data.api_client, address)
        .await
        .map_err(|err| {
            vlog::warn!(
                "Internal Server Error: '{}'; input: ({}, {}, {})",
...
async fn handle_get_tx_by_hash(
    ...
    let unconfirmed_op = get_unconfirmed_op_by_hash(&data.api_client, hash)
        .await
        .map_err(|err| {
            vlog::warn!(
                "Internal Server Error: '{}'; input({})",
