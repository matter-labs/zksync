...
pub(crate) struct Dispatcher<API: TokenPriceAPI, INFO, WATCHER> {
...
impl<API, INFO, WATCHER> Dispatcher<API, INFO, WATCHER>
where
...
mod tests {
    ...
    async fn dispatch() {
        ...
        let dispatcher = Dispatcher::<CoinGeckoAPI, TickerInfo, UniswapTokenWatcher> {
            tickers: vec![],
            channels: senders,
