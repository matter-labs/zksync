...
use crate::{
    leader_election::LeaderElectionSchema,
    StorageProcessor,
};
...
fn vote_for_leader() {
    ...
    db_test(conn.conn(), || {
        ...
        conn.leader_election_schema().vote_for_leader("foo", leader_period).unwrap();
        conn.leader_election_schema().vote_for_leader("bar", leader_period).unwrap();
