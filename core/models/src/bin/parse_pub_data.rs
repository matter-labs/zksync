...
fn main() {
    ...
    while !unparsed_data.is_empty() {
        ...
        let op_data_len = FranklinOp::public_data_length(op_type).expect("wrong op type");
        assert!(
        ...
        let op = FranklinOp::from_public_data(&current_op).expect("failed to parse pubdata");
        println!("{:#?}", op);
