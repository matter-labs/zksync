...
async fn transfer_eth_to(to: H160) {
    ...
    let credentials =
        WalletCredentials::from_eth_pk(main_eth_address, main_eth_private_key).unwrap();
...
async fn simple_workflow() -> Result<(), anyhow::Error> {
    ...
    let provider = Provider::new(Network::Localhost);
    let credentials = WalletCredentials::from_eth_pk(eth_address, eth_private_key).unwrap();
