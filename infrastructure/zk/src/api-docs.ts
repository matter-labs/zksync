...
function pasteAllFilesInOne() {
    ...
    const groupsFiles = fs.readdirSync(path.join(getDirPath(), 'groups'));
    for (let file of groupsFiles) {
        const data = fs.readFileSync(path.join(getDirPath(), 'groups', file), 'utf-8');
        replaceObject[file.replace('.apib', '') + 'Endpoints'] = data;
    }

    const typesFiles = fs.readdirSync(path.join(getDirPath(), 'types'));
    for (const file of typesFiles) {
        const data = fs.readFileSync(path.join(getDirPath(), 'types', file), 'utf-8');
...
export async function compileDocs() {
    ...
    const inputPath = path.join(getDirPath(), 'documentation.apib');
    const outputPath = path.join(getDirPath(), 'index.html');
    await utils.spawn(`aglio -i ${inputPath} -o ${outputPath}`);
}

export async function testDocs() {
    await compileApibForTest();
    await utils.spawn(`dredd`);
}

export const command = new Command('api-docs').description('api documentation management');

command
    .command('compile')
    .description('compile api documentation')
    .option('--test', 'test api documentation')
