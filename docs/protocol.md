...
| PackedTxAmount     | 5        | [Parameters](#amount-packing) | Packed transactions amounts are represented with 40 bit (5 byte) values, encoded as mantissa _ 10^exponent where mantissa is represented with 35 bits, exponent is represented with 5 bits. This gives a range from 0 to 34359738368 _ 10^31, providing 10 full decimal digit precision.                             |
...
| StateAmount        | 16       | BE integer                    | State amount is represented as uint128 with a range from 0 to ~3.4 _ 10^38. It allows to represent up to 3.4 _ 10^20 "units" if standard Ethereum's 18 decimal symbols are used. This should be a sufficient range.                                                                                                  |
...
```
08000000040002000000000000000002c68af0bb1400000012080910111213141516171819202122233425262800000000
```

Reads as: account #4 initiates a transfer of token #2 amount 0x000000000000000002c68af0bb140000 for fee packed in representation 0x0012 for the Rollup account which has Ethereum account address 0x0809101112131415161718192021222334252628.
