  ...
  for (let len = 1; len <= 32; ++len) {
    ...
    for (let variant of ["MusigPedersen", "MusigSha256"]) {
      ...
      let resp = await Axios.post(
        specTestServer + "/check_signature",
        req
      ).then(reps => reps.data);
      if (!resp.correct) {
...
async function addressCheck() {
  ...
  let resp = await Axios.post(specTestServer + "/address", {
    pub_key: serializePointPacked(keys.publicKey).toString("hex")
  }).then(reps => reps.data);
...
async function txSignatureCheck() {
  let keys = new WalletKeys(new BN(crypto.randomBytes(16)));
  let transfer = {
    from: `0x${pubkeyToAddress(keys.publicKey)}`,
    to: `0x${crypto.randomBytes(addressLen)}`,
    token: 2,
  ...
  let req = FranklinProvider.prepareTransferRequestForNode(
    transfer,
    transferSign
  );
  let resp = await Axios.post(specTestServer + "/check_tx_signature", req).then(
    reps => reps.data
  );
  if (!resp.valid) {
  ...
  let withdraw = {
    account: `0x${pubkeyToAddress(keys.publicKey)}`,
  ...
  resp = await Axios.post(specTestServer + "/check_tx_signature", req).then(
    reps => reps.data
  );
  if (!resp.valid) {
...
async function fullExitSignatureCheck() {
  ...
  let resp = await Axios.post(
    specTestServer + "/check_full_exit_signature",
    req
  ).then(reps => reps.data);
  if (!resp.valid) {
  ...
  if (
    !Buffer.from(resp.account_address.substr(2), "hex").equals(
      pubkeyToAddress(keys.publicKey)
    )
  ) {
