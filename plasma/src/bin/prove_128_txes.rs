...
fn main() {
    ...
    let cirtuit_params = generate_random_parameters(instance_for_generation, rng).unwrap();
    ...
    let start = PreciseTime::now();
    let proof = create_random_proof(instance_for_proof, &cirtuit_params, rng).unwrap();
