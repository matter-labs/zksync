...
fn main() {
    ...
    let start_prep = PreciseTime::now();
    let prepared_prover: PreparedProver<Bn256> = prepare_prover(instance_for_proof).unwrap();
    println!("prover preparation in {} s", start_prep.to(PreciseTime::now()).num_milliseconds() as f64 / 1000.0);

    println!("creating proof...");
    let start = PreciseTime::now();
    let proof = prepared_prover.create_random_proof(&circuit_params, rng).unwrap();
