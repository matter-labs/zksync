...
use plasma::models::circuit::{DepositRequest, ExitRequest, TransferTx};
use plasma::models::{self, Block, BlockData};

fn convert_transfer(transactions: &Vec<models::TransferTx>) -> Result<Vec<Vec<bool>>, String> {
    transactions
        .iter()
        .map(|tx| TransferTx::try_from(tx).map(|tx| tx.public_data_into_bits()))
        .collect()
}
...
fn convert_deposit(transactions: &Vec<models::DepositTx>) -> Result<Vec<Vec<bool>>, String> {
    transactions
        .iter()
        .map(|tx| DepositRequest::try_from(tx).map(|tx| tx.public_data_into_bits()))
        .collect()
}
...
fn convert_exit(transactions: &Vec<models::ExitTx>) -> Result<Vec<Vec<bool>>, String> {
    transactions
        .iter()
        .map(|tx| ExitRequest::try_from(tx).map(|tx| tx.public_data_into_bits()))
        .collect()
}
