...
export class Wallet {
  ...
  constructor(
    seed: Buffer,
    public provider: SidechainProvider,
    public ethWallet: ethers.Signer
  ) {
    const { privateKey } = privateKeyFromSeed(seed);
    this.walletKeys = new WalletKeys(privateKey);
    this.address = `0x${pubkeyToAddress(this.walletKeys.publicKey).toString(
      "hex"
    )}`;
  }
  ...
  async withdrawFromContractToETHAddress(
    ...
    if (token.id === 0) {
      return await sidechainMainContract.withdrawETH(amount, {
        gasLimit: 200000
      });
    } else {
