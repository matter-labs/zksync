...
export async function verifyERC1271Signature(
    ...
    const prefixed_message = `\x19Ethereum Signed Message:\n32${utils.keccak256(message)}`;
    const sign_message = utils.keccak256(prefixed_message);

    const eip1271 = new ethers.Contract(address, IEIP1271_INTERFACE, signerOrProvider);
    const eipRetVal = await eip1271.isValidSignature(utils.hexlify(sign_message), signature);
