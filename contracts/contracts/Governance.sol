...
contract Governance {
    ...
    mapping(address => bool) internal trustedAddresses;
    ...
    // Params:
    // - _token - token address
    function addCToken(address _cToken, address _token) external {
        requireGovernor();
        require(
            tokenIds[_token] != 0,
            "gan11"
    ...
    // Check if sender is active validator
    function isActiveValidator(address _address) public view returns (bool) {
        return validatorsInfo[_address].isActive;
    }
    
    function getValidatorId(address _address) public view returns (uint256) {
        return validatorsInfo[_address].id;
    }
    ...
    ) external view returns (uint256 signersCount, bool result) {
    ...
    function supplyErc20(address _tokenAddress,
                         uint256 _amount,
                         address _validator)
    external
    {
    ...
    function borrowToTrustedAddress(address _tokenAddress, uint256 _amount) external returns(bool result) {
        address tokenId = validateTokenAddress(_tokenAddress);
        require(
            trustedAddresses[msg.sender],
            "grr11"
        ); // grr11 - not trusted address
        uint256 supply = funds[tokenId];
        if (supply < _amount) {
            return false;
        }
        if (tokenId == 0) {
            ...
            require(
                IERC20(_tokenAddress).transfer(swiftExits, _amount),
                "sstt11"
            ); // sstt11 - token transfer out failed
        }
        funds[tokenId] -= _amount;
    ...
    function repayBorrowInERC20(address _tokenAddress,
                                uint256 _amount)
    external
    {
        address tokenId = validateTokenAddress(_tokenAddress);
    ...
    /// @notice Transfers specified amount of ERC20 token into contract
    /// @param _amount Amount in ERC20 tokens
    function punishValidator(address _validator, address _tokenAddress, uint256 _amount)
    external
    payable
    {
        funds[0] = funds[0].add(_amount);
