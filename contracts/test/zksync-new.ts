...
describe("Token", function () {
    ...
    before(async function () {
        ...
        const zeroBlockStoredHash = ethers.utils.keccak256(
            ethers.utils.solidityPack(["bytes32", "bytes32", "bytes32"], [zeroBlock.processableOnchainOperationsHash, zeroBlock.stateHash, zeroBlock.commitment])
        );
        await zkSync.initialize(ethers.utils.defaultAbiCoder.encode(["address", "address", "bytes32"], [gov.address, ethers.constants.AddressZero, zeroBlockStoredHash]));
    });
