...
describe('zkSync signature verification unit tests', function () {
    ...
    it('pubkey hash signature EIP712 verification success', async () => {
        ...
        const wallet = new ethers.Wallet('0xd26b03c9b31e2b4d09b4761ea86b3e0a32dd23b3886c7111280d73f0c34c1841');
        const message = {
            pubKeyHash: pubkeyHash,
            nonce: nonce,
            accountId: accountId
        };
        const signature = await wallet._signTypedData(EIP712Domain, types, message);
        ...
        const { result } = await getCallRevertReason(
            async () =>
                await testContract.changePubkeySignatureCheckEIP712(
                    { accountId, owner: wallet.address, nonce, pubKeyHash: pubkeyHash },
                    witness
                )
        );
        expect(result).eq(true);
