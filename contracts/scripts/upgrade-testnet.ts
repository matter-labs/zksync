...
(async () => {
    ...
    parser.addArgument("--cancelPreviousUpgrade", {required: false, help: "cancels pending upgrade", action: "storeTrue"});
    ...
    if (args.cancelPreviousUpgrade) {
        const cancelUpgradeTx = await upgradeGatekeeper.cancelUpgrade({gasPrice, gasLimit: 500000, nonce: args.nonce});
        if (args.nonce != null) {
            ++args.nonce;
        }
    ...
    const startUpgradeTx = await upgradeGatekeeper.startUpgrade(upgradeTargets, {gasPrice, gasLimit: 500000, nonce: args.nonce});
    if (args.nonce != null) {
        ++args.nonce;
    }
    ...
    const startPreparationUpgradeTx = await upgradeGatekeeper.startPreparation({gasPrice, gasLimit: 500000, nonce: args.nonce});
    if (args.nonce != null) {
        ++args.nonce;
    }
    ...
    const finishUpgradeTx = await upgradeGatekeeper.finishUpgrade(initArgs, {gasPrice, gasLimit: 500000, nonce: args.nonce});
