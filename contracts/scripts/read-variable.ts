...
async function getStorageAt(address: string, slot: BigNumber): Promise<string> {
    if(!cash.has(slot))
        cash.set(slot, (await provider.getStorageAt(address, slot)).substr(2, 64));
...
function hexToSignedNumber(hex: string): string {
    ...
    if (parseInt(hex.substr(0,1),16) > 7) {
        bn = BigInt('0b' + bn.toString(2).split('').map(i => {
            return '0' === i ? 1 : 0
        }).join('')) + BigInt(1);
        bn = -bn;
    }
...
async function readStruct(slot: BigNumber, address: string, type: string): Promise<object> {
    ...
    for (const member of types[type].members) {
        result[member.label] = await readVariable(slot.add(Number.parseInt(member.slot, 10)), member.offset, address, member.type);
    }
    return result;
}
...
async function readPartOfArray(slot: BigNumber, address: string, type: string, params: string[]): Promise<any> {
    ...
    if(Number.parseInt(types[baseType].numberOfBytes, 10) < 32){
        ...
        return readPartOfVariable(slot.add(index*Number.parseInt(types[baseType].numberOfBytes, 10)/32), 0, address, baseType, params);
}

function parseKey(key: string, type: string): string {
    if(type === "t_bool"){
        if(key === "false" || key === "0")
            return numberTo32Hex("0");
        else
            return numberTo32Hex("1");
    }
    if(type === "t_address" || type === "t_address_payable") {
        if(key.length === 42)
            return key.substring(2, key.length).padStart(64, '0');
        else
...
async function readPartOfMap(slot: BigNumber, address: string, type: string, params: string[]): Promise<any> {
    ...
    slot = BigNumber.from(ethers.utils.keccak256(hexStringToByte(parseKey(key, keyType) + numberTo32Hex(slot.toString()))));
    return readPartOfVariable(slot, 0, address, valueType, params);
}

async function readPartOfVariable(slot: BigNumber, shift: number, address: string, type: string, params: string[]): Promise<any>{
