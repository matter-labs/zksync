...
pub struct NonInclusion<'a, E: JubjubEngine> {
    ...
    // Witnesses
    pub witness: Vec<Option<BlockWitness>>,
}
...
impl<'a, E: JubjubEngine> Circuit<E> for NonInclusion<'a, E> {
    fn synthesize<CS: ConstraintSystem<E>>(self, cs: &mut CS) -> Result<(), SynthesisError>
    {
        ...
        let zero = Vec::<bool>::with_capacity(self.leaf_hash_length);
        zero.resize(boolean::Boolean::Constant(false), self.leaf_hash_length);

        for (i, w) in witness.into_iter().enumerate() {
            // allocate public input
            let mut proof_input = AllocatedNum::alloc(
                cs.namespace(|| format!("root number {}", i)),
                || Ok(*w.get()?)
            proof_input.inputize(cs.namespace(|| format!("input for root number {}", i))?;
