...
impl<'a, E: JubjubEngine> Circuit<E> for NonInclusion<'a, E> {
    fn synthesize<CS: ConstraintSystem<E>>(self, cs: &mut CS) -> Result<(), SynthesisError>
    {
        ...
        level.inputize(cs.namespace(|| "level input"))?;

        // index is expected to be just a coin number
        let mut level_bits = level.into_bits_le(
            cs.namespace(|| "level bits")
        )?;
        level_bits.truncate(self.tree_depth);
        ...
        for bit in level_bits.clone().into_iter() {
        ...
        for (i_witnesses, w) in self.witness.into_iter().enumerate() {
            ...
            let mut proof_input = AllocatedNum::alloc(
                cs.namespace(|| format!("root number {}", i_witnesses)),
                || Ok(*w.root.get()?)
            )?;

            proof_input.inputize(
                cs.namespace(|| format!("input for root number {}", i_witnesses))
            )?;
            ...
            for (i_tree_levels, (e, level_bit)) in audit_path.clone().into_iter().zip(level_bitmask.clone().into_iter()).enumerate() {
                let cs = &mut cs.namespace(|| format!("merkle tree hash {}", i_tree_levels));
                ...
                boolean::Boolean::enforce_equal(
                    cs.namespace(|| "position bit is equal the start of the interval field bit"),
                    &cur_is_right, 
...
fn test_non_inclusion_proof() {
    ...
    for _ in 0..NUMBER_OF_BLOCKS_TO_PROVE {
        ...
        let slice_len = 1 << start_of_slice;
